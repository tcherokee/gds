<script lang="ts">
	...
	const gameInFullscreenToggle = () => {
		...
		fullscreen ? (gamePlayerClass = 'game-player-fullscreen') : (gamePlayerClass = 'game-player')
	}
...
<div class="flex flex-col justify-center rounded-t-lg -mx-3 md:mx-0">
    <div class={gamePlayerClass}>
        <div class="float-close-btn">
            <div class="buttons">
                ...
                <button type="button" class="btn btn-link" on:click={gameInFullscreenToggle}>
        ...
        {#if !startGame}
            <div class="start-game-overlay">
                {#if data?.attributes?.isGameDisabled}
                    ...
                    <h3 class="mt-5 text-white mb-4">{data.attributes?.gameDisableText ? data.attributes?.gameDisableText : ''}</h3>
                {:else}
    ...
    >
        <div class="provider-meta p-2.5 md:p-0 flex order-1 bg-white grow md:grow-0">
        ...
        >
            ...
            >
                <div class="ml-0 md:ml-4">
                    <ReadOnlyRatings avgRating={data?.attributes?.ratingAvg} showVotes={true} ratingClasses="flex-col" />
                </div>
