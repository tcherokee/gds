<script lang="ts">
	...
	import qs from 'qs';
// Import Types
    import type { BonusLabels, SportFilters, Toggles, TranslationData } from '../../../interfaces/common/types';
    // Import QS Function
...
// Import Stores
    import { bonusAmount, sportQsStore, sportVariables, wageringReqAmount } from "../../../stores/sports";
    import { bonusLabels, conditions, sort } from "../../../stores/filters";


	// Valid Keys for the Sport Filters Array to validate and squash typescript errors
    const validKeys: (keyof SportFilters)[] = ["limit", "sort", "providers", "ids", "bonusKey", "condition", "amount", "wagering", "speed"];
	...
	$: toggleOrder = $sportVariables.sort ? $sportVariables.sort?.split(":")[1] : 'desc'
	...
	const changeSortOrder = (value: string) => {
		let [key,] = $sportVariables.sort ? $sportVariables.sort?.split(":") : ''

		
		if (key.length > 0) {
			const newSortValue = key + ":" + value

			sportVariables.setKey('sort', newSortValue)
        
			// Set QS Query String to Get Updated Casinos
			const query = qs.stringify(sportsQs($sportVariables), {encodeValuesOnly: true})
			
			// Set Qs Store to Query Value
			sportQsStore.set(`?${query}`)
		}
	}
    ...
    const handleFilterChange = (key: string, value: string | number) => {
		...
		validKeys.includes(key as keyof SportFilters) && sportVariables.setKey(key as keyof SportFilters, value)
    ...
    const clearFilterOptions = (e: Event, key: string, value: string | number) => {
		...
		// Set Sport IDs to an empty array or it will override any filtering
		$sportVariables.ids.length > 0 && sportVariables.setKey('ids', [])
					...
					<div>
						<div
							class="
								...
								{$sportVariables.bonusKey ? 'bg-filter-alternate-toggle' : 'bg-white/[.5]'}
							"
							aria-expanded="false"
							role="button"
							tabindex="-1"
						...
						>
							<span class="uppercase text-xs pointer-events-none">
								{$sportVariables.bonusKey ? getLabelByValue($bonusLabels, $sportVariables.bonusKey) : translationStore?.welcomeBonus }
							</span>
							{#if $sportVariables.bonusKey}
						...
						<div class="flex gap-2 py-1" role="none">
							...
							{#each bonusTypeKeys as key}
								<button
									...
									{...($bonusLabels[key]?.value === $sportVariables.bonusKey) ? {'data-ui':'active'} : {}}
					...
					<div>
						<div
							class="
								...
								{$sportVariables.condition ? 'bg-filter-alternate-toggle' : 'bg-white/[.5]'}
								{$sportVariables.bonusKey ? '!text-black cursor-pointer' : ''}
							"
							aria-expanded="false"
							role="button"
							tabindex="-1"
							{...(toggles.toggleConditions ? { open:'' } : {})}
							on:click={() => $sportVariables.bonusKey && handleToggle('toggleConditions')}
						>
							<span class="uppercase text-xs pointer-events-none">
								{$sportVariables.condition ? getLabelByValue($conditions, $sportVariables.condition) : translationStore?.condition }
							</span>
							{#if $sportVariables.condition}
						...
						<div class="flex gap-2 py-1" role="none">
							{#each conditionsKeys as key}
								<button
									...
									tabindex="-1"
									id="mobile-menu-item-0"
									{...($conditions[key]?.value === $sportVariables.condition) ? {'data-ui':'active'} : {}}
						...
						<div>
							<div
								class="
									...
									{$sportVariables.amount ? 'bg-filter-alternate-toggle' : 'bg-white/[.5]'}
									{$sportVariables.bonusKey ? '!text-black cursor-pointer' : ''}
								"
								aria-expanded="false"
								role="button"
							...
							>
								<span class="uppercase text-xs pointer-events-none">
									{$sportVariables.amount ? $sportVariables.amount : translationStore?.bonusAmount }
								</span>
								{#if $sportVariables.amount}
							...
							<div class="flex flex-col gap-y-2 py-1" role="none">
								{#each $bonusAmount as bonus}
									<button
										...
										tabindex="-1"
										id="mobile-menu-item-0"
										{...(Number(bonus) === Number($sportVariables.amount)) ? {'data-ui':'active'} : {}}
						...
						<div>
							<div
								class="
									...
									{$sportVariables.wagering ? 'bg-filter-alternate-toggle' : 'bg-white/[.5]'}
									{$sportVariables.bonusKey ? '!text-black cursor-pointer' : ''}
								"
								aria-expanded="false"
								role="button"
							...
							>
								<span class="uppercase text-xs pointer-events-none">
									{$sportVariables.wagering ? $sportVariables.wagering : translationStore?.wagering }
								</span>
								{#if $sportVariables.wagering}
							...
							<div class="flex flex-col gap-y-2 py-1" role="none">
								{#each $wageringReqAmount as wager}
									<button
										...
										tabindex="-1"
										id="mobile-menu-item-0"
										{...(Number(wager) === Number($sportVariables.wagering)) ? {'data-ui':'active'} : {}}
				...
				<div class="relative basis-60">
					<div id="desktop-menu-0" class="relativ w-full inline-block text-left">
						<div class="w-full inline-flex rounded-md shadow-sm">
							...
							>
								<span class="uppercase text-xs pointer-events-none">
									{$sportVariables.sort ? getSortLabelByValue($sportVariables.sort) : translationStore?.welcomeBonus }
								</span>
							</div>
							<button
								type="button"
							...
							<div class="flex flex-col gap-y-2 py-1" role="none">
								{#each sortKeys as key}
									<button
										...
										tabindex="-1"
										id="sort-keys"
										{...($sportVariables.sort === $sort[key].value) ? {'data-ui':'active'} : {}}
